...
if device == '아이패드':
    ...
    if show_margin:
        ...
        # 매입 전략 시나리오 선택
        SCENARIO = {
            '보수 (재고 확보)': {'pro': 0.08, 'air': 0.10, 'mini': 0.13, 'basic': 0.15},
            '기본 (균형)': {'pro': 0.10, 'air': 0.13, 'mini': 0.17, 'basic': 0.20},
            '공격 (마진 우선)': {'pro': 0.12, 'air': 0.15, 'mini': 0.20, 'basic': 0.25},
        }
        ...
        # 부대비용 (사용자 입력)
        운영비_m = st.number_input("운영비 (원) — 인건비, 검수비, 물류비", min_value=0, value=30000, step=5000, key='운영비_m')
        repair_cost_m = st.number_input("수리비 (원)", min_value=0, value=0, step=10000, key='repair_cost_m')
        shipping_cost_m = st.number_input("배송비 (원)", min_value=0, value=5000, step=1000, key='shipping_cost_m')

        # 총 비용 = 매입가 + 운영비 + 수리비 + 배송비
        total_cost = 최종매입가 + 운영비_m + repair_cost_m + shipping_cost_m

        # 마진 = 판매 예상가 - 총 비용 (마진은 결과로 나옴)
        ...
        with col_m3:
            st.metric("순마진", f"{순마진:,.0f}원  (▲{순마진율:.1f}%)")
        with col_m4:
        ...
        마진율_즉시 = (마진_즉시 / sell_price * 100) if sell_price > 0 else 0
